...
- [x] 向量化实验层已实现到 SQLite sidecar、fake provider 和本地 vector top-k 召回；craft 默认 auto on，未接入 provider/index 时自动跳过。
...
- [x] 真实 LLM 候选生成第一版：OpenAI-compatible、默认 auto on、只进入候选层。
...
- type + token 的低置信补充召回。
- embedding 召回默认 auto on；仅当 `CraftOptions.use_vectors=True` 且实际传入 vector index/provider 时启用，否则自动跳过。
...
### Phase 5：向量化实验层（已完成基础 sidecar 和本地 top-k，默认 auto on）
...
- craft 默认进入 vector auto 模式；只有实际接入 vector index/provider 且存在 active embedding 时才会把 vector top-k 作为候选证据。
...
- `RuleSynthesizerEngine` 已可选接入 vector index，把 object/recipe vector top-k 作为候选证据；未传入 index/provider 时自动跳过。
...
### Phase 6：LLM 候选生成（已完成第一版，默认 auto on）
...
- 已接入 object vector top-k：使用 candidate embedding 查询相似 active object，默认 auto on。
- 已接入 recipe vector top-k：使用 request embedding 查询相似 active recipe，并把 recipe result 作为候选证据，默认 auto on。
- `craft --no-vectors` 可在开发排障时关闭 vector 召回。
...
-> LLM candidates + rule fallback + local ranker
-> vector top-k objects/recipes when provider/index is available
...
flowchart TD
    ...
    J --> K["候选生成<br/>默认 LLM 候选 + 规则兜底"]
    K --> L{"vector provider/index<br/>是否可用？"}
...
3. 保留 fake vector 测试看护和 `--no-vectors` 排障入口。
...
| Phase 5 | 向量化实验 | embedding 去重率、stale 检测、auto 跳过回归测试 |
| Phase 6 | LLM 候选（第一版已接入） | schema 合格率、fallback 率、candidate hit |
| Phase 7 | 混合检索（后置实验） | text recall hit、vector auto top-k hit、错误分布 |
